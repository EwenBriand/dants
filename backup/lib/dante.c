...
void add_neighboors_to_list(node_t *mother, maze_t *maze, int *out, node_t **list)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (maze->maze[tmp_y][tmp_x] == '*')
            list_add(new_node(build_parents_from_mother(mother, tmp_x, tmp_y), mother->p_len + 1, NULL, maze), list);
        if (tmp_x == 0 && tmp_y == 0)
            *out = 1;
    }
}
...
{
    int *zero_parents[2] = malloc(sizeof(int *));
    zero_parents[0] = 0;
    zero_parents[1] = 0;
    node_t *available_nodes = new_node(zero_parents, 1, NULL, maze);
    node_t *tmp = NULL;
    int out = 0;
